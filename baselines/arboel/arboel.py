...
from belb import (ENTITY_TO_KB_NAME, SYMBOL_CODE, Annotation, AutoBelbCorpus,
                  AutoBelbKb, Entities, Example, Queries, Tables)
...
class ArboEl(Model):
    ...
    def __init__(self, *args, add_species: bool = False, **kwargs):
            ...
            for split, documents in data.items():
                ...
                with open(split_outfile_mentions, "w") as out_fp_mentions, open(
                    ...
                    for document in documents:
                        if self.ab3p:
                            document = self.expand_abbreviations(
                                example=document,
    ...
    def convert_kbs(self, shard_size: int = int(1e6)):
        ...
        for spec in self.kbs:
            ...
            kbs_dir = "kbs"
            if self.add_species and name == Kbs.NCBI_GENE.name:
                kbs_dir += "_species"

            outdir = os.path.join(self.in_dir, kbs_dir, name)

            if subset is not None:
                outdir = os.path.join(outdir, subset)
            ...
            with kb as handle:
                if self.add_species and name == Kbs.NCBI_GENE.name:
                    fi = kb.schema.get(Tables.FOREIGN_IDENTIFIERS)
                    identifier_to_species = {}
                    for row in handle.query(select(fi)):
                        identifier_to_species[row["identifier"]] = row["name"]
                ...
                for row in handle.query(query):
                    ...
                    line = {"document_id": str(identifier)}
                    ...
                    if len(names) > 0:
                        text += f"{symbol} ("
                        if self.add_species:
                            species_identifier = parsed_row["foreign_identifier"]
                            species = identifier_to_species[species_identifier]
                            text += f" {species}: "
                        text += " ; ".join(names)
                        text += " )"
                    ...
                    if len(shard) < shard_size:
                        ...
                        with open(os.path.join(outdir, f"shard{idx}.json"), "w") as fp:
                            for line in shard:
...
def main():
    ...
    parser.add_argument(
        "--add_species",
        action="store_true",
        help="Add species name into entity description (only for NCBI Gene)",
    )
