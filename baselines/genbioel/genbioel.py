...
from models.utils import (convert_to_disambiguated_name, get_argument_parser,
                          load_json, save_json)
...
DISAMB_PUNCTUATION = re.compile(r"[,.;{}[\]+\-_*/?!`\"\'=%><]")


def preprocess_name(
    name: str, lowercase: bool = False, punctuation: re.Pattern = PUNCTUATION
):
    """
...
class GenBioEl(Model):
    ...
    def __init__(
        self, *args, lowercase: bool = False, disambiguated: bool = False, **kwargs
    ):
    ...
    def convert_kbs(self):
        ...
        for spec in self.kbs:
            ...
            id_to_disambiguated_names = defaultdict(set)
            with kb as handle:
                for row in handle.query(query):
                    identifier = row["identifier"]
                    row = convert_to_disambiguated_name(
                        row=row, connector=kb.queries.connector
                    )
                    disambiguated_name = preprocess_name(
                        name=row["unique_name"],
                        lowercase=self.lowercase,
                        punctuation=DISAMB_PUNCTUATION,
                    )
                    ...
                    id_to_disambiguated_names[identifier].add(disambiguated_name)

            # save_json(
            #     path=os.path.join(kb_outdir, "target_kb.json"),
            #     item={k: list(v) for k, v in id_to_names.items()},
            # )
            save_json(
                path=os.path.join(kb_outdir, "target_dkb.json"),
                item={k: list(v) for k, v in id_to_disambiguated_names.items()},
                indent=1,
            )
    ...
    def convert_corpora(self):
        ...
        for corpus_name, entity_type in self.corpora:
            ...
            file_name = "target_dkb.json" if self.disambiguated else "target_kb.json"

            id_to_names = load_json(os.path.join(*parts, file_name))
    ...
    def convert_kbs_to_trie(self):
        ...
        for spec in self.kbs:
            ...
            for kb_name, trie_name in [
                # ("target_kb.json", "trie.pkl"),
                ("target_dkb.json", "dtrie.pkl"),
            ]:
                id_to_names = load_json(os.path.join(kb_outdir, kb_name))
                trie = Trie(
                    tokenize_entities(
                        set(n for id, names in id_to_names.items() for n in names)
                    )
                )
                trie_out = os.path.join(kb_outdir, trie_name)
    ...
    def create_input(self):
        ...
        self.convert_corpora(disambiguated=self.disambiguated)
    ...
    def parse_output(
        ...
        hexdigests_path = os.path.join(
            self.in_dir, "runs_disamb", dir_name, "test.hexdigests.txt"
        )
